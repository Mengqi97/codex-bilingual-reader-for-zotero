...
test("menus, item pane, and manual PDF width adjustment expose the requested controls", async () => {
  ...
  assert.match(main, /Zotero\.Reader\.open\(bilingualPdf\.id\)/);
  ...
  assert.match(main, /async function bilingualPdfPreviewDataURL\(attachment, doc\)/);
  ...
  assert.match(main, /await refreshPreview\(\);/);
});
